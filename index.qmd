---
title: "Reporte regional de atenciones de urgencia (Pediatría)"
...
Esta información es especialmente relevante durante los meses de otoño e invierno, dado el aumento de consultas y circuclación viral en la población. <br>
...
respiratory_cause_ped <- data_joined |>
  filter(
...
respiratory_cause_ped$grupo_etario <- factor(respiratory_cause_ped$grupo_etario,
  labels = c("menores_1", "de_1_a_4", "de_5_a_14")
)

respiratory_cause_ped$nombre_region.x <- factor(respiratory_cause_ped$nombre_region.x
)
```
...
regions <- unique(respiratory_cause_ped$nombre_region.x)

for (region in regions) {
  data_region <- respiratory_cause_ped[respiratory_cause_ped$nombre_region.x == region, ]
  ...
  plot <- data_region |>
    ...
    theme_grey() +
    labs(
      title = "Atenciones de urgencia Pediátricas por causas respiratorias. 2023",
